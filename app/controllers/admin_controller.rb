...
class AdminController < ApplicationController
  ...
  def index
    ...
    @quick_stats = {
      sales: Order.where(created_at: today..Time.zone.now).count,
      revenue: Order.where(created_at: today..Time.zone.now).sum(:total),
      avg_sale: Order.where(created_at: today..Time.zone.now).average(:total),
      per_sale: OrderProduct.joins(:order).where(orders: { created_at: today..Time.zone.now }).average(:quantity)
    }
